...
def carregar_aba(nome_aba, colunas_padrao):
    try:
        ...
        # Limpa eventuais linhas 100% vazias
...
def registrar_lote_enviado(leads_para_gravar, corretor_destino, tipo_lead):
    ...
    if df_atual.empty:
        ...
        # Identifica duplicados e incrementa total_cobrancas
        keys_novas = set(df_novos['lead_key'])
        df_mantidos = df_atual[~df_atual['lead_key'].isin(keys_novas)].copy()
        
        # Para os que já existiam, busca cobranças anteriores + 1
...
if arquivo_atual:
    ...
    st.info("Faça o upload do relatório diário na barra lateral para iniciar.")    if "etapa_ao_enviar" not in colunas_existentes:
