...
def exportar_backup_completo():
    conn = sqlite3.connect(DB_FILE)
    df_envios = pd.read_sql_query("SELECT * FROM controle_envios", conn)
    conn.close()
    return df_envios.to_csv(index=False).encode('utf-8')
...
def restaurar_backup(df_backup):
    ...
    for _, row in df_backup.iterrows():
        c.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(lead_key) DO UPDATE SET
                corretor_cobrado = excluded.corretor_cobrado,
                data_envio = excluded.data_envio,
                tipo_lead = excluded.tipo_lead,
                etapa_ao_enviar = excluded.etapa_ao_enviar,
                total_cobrancas = excluded.total_cobrancas
        ''', (
            str(row.get('lead_key', '')),
            str(row.get('nome', '')),
            str(row.get('celular', '')),
            str(row.get('corretor_cobrado', '')),
            str(row.get('corretor_original', '')),
            str(row.get('tipo_lead', '')),
            str(row.get('etapa_ao_enviar', '')),
            str(row.get('data_envio', '')),
            int(row.get('total_cobrancas', 1))
        ))
    conn.commit()
    conn.close()
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 🛡️ Segurança do Histórico")
with st.sidebar.expander("Backup & Restauração", expanded=False):
    csv_backup = exportar_backup_completo()
    st.download_button(
        label="💾 Baixar Backup do Histórico (.csv)",
        data=csv_backup,
        file_name=f"backup_retrabalho_{datetime.datetime.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",
        help="Baixe este arquivo para guardar um backup de tudo o que foi cobrado e redistribuído."
    )
    
    arquivo_restaurar = st.file_uploader("Restaurar histórico anterior (.csv):", type=["csv"], key="restaurar_csv")
    if arquivo_restaurar is not None:
        if st.button("Subir e Restaurar Backup"):
            try:
                df_rest = pd.read_csv(arquivo_restaurar)
                restaurar_backup(df_rest)
